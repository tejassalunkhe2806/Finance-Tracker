            ...
            <>
              ...
              {activeTab === 'accounts' && (
                <div className="space-y-6">
                  ...
                  {accounts.length === 0 ? (
                    <div className={`border-2 border-dashed rounded-2xl p-12 text-center ${isDark ? 'border-slate-800 bg-slate-900/40' : 'border-slate-200 bg-slate-50/40'}`}>
                      <p className="text-slate-400 dark:text-slate-500 text-sm mb-4">You haven't added any accounts yet.</p>
                      <button
                        onClick={() => setIsAccModalOpen(true)}
                        className={`text-xs font-semibold px-5 py-2.5 rounded-lg shadow-sm transition-all ${isDark ? 'bg-white hover:bg-slate-200 text-slate-900' : 'bg-slate-900 hover:bg-slate-850 text-white'}`}
                      >
