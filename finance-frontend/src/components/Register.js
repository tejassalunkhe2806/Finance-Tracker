...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Your account was created! Redirecting to login...');
      setTimeout(() => {
        navigate('/login');
      }, 1200);
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">Join Aethera to manage your wealthcompanion.</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          ...
          {success && (
            <div className="mb-5 p-3 bg-green-50 border border-green-200 text-green-600 text-xs rounded-lg flex items-center gap-2">
              <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-slate-700 text-xs font-semibold mb-1.5 uppercase tracking-wider">Full Name</label>
              <input
                type="text"
